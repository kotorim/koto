...
float3 MColorM = {1.00,1.00,1.00};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorM = {1.00,1.00,1.00};//ボトムライトの色。

float LightIntensityM = 1.0;//メインライトの全体の強さ。1で減少無し。
float TIntensityM = 1.0;//トップライトの強さ。1.0で強さの変化なし。
float MIntensityM = 1.0;//ミドルライトの強さ。1.0で強さの変化なし。
float UIntensityM = 1.0;//ボトムライトの強さ。1.0で強さの変化なし。

/////////////////////////////
//照り返し環境光パラメーター
float3 TColorA = {1.00,1.00,1.00};//トップライトの色。//すべて1で白。すべて0で黒。
float3 MColorA = {1.00,1.00,1.00};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorA = {1.00,1.00,1.00};//ボトムライトの色。

float LightIntensityA = 0.0;//照り返し環境光の全体の強さ。1で減少無し。
float TIntensityA = 1.0;//トップライトの強さ。1.0で強さの変化なし。
float MIntensityA = 0.2;//ミドルライトの強さ。1.0で強さの変化なし。
float UIntensityA = 1.0;//ボトムライトの強さ。1.0で強さの変化なし。
float AmbientBalance = 0.0;//環境光を加算か乗算か。１で加算。０で乗算

/////////////////////////////
//ハイライトパラメーター
int SpecularON = 0;//1で元のスペキュラーを使用。0で別計算のハイライトを使用。
float HilightPower = 0.20;//ハイライトの強さ。
float3 HiColor = {1.00,1.00,1.00};//ハイライトの色。すべて1で白。
float HilightGlow = 0.4;//ハイライトの広がり。数値を上げれば、ハイライトが狭くなる。基本2～3ぐらい。

/////////////////////////////
//エッジライトパラメーター(擬似バックライト)
float3 EColor = {1.0,0.9,0.7};
float LightIntensityE = -0.2;//エッジライト強さ自体の強さ
float AttenuationC = 0.0;//明るい所のエッジライト減少。０で減少無し。
float AttenuationD = 0.0;//ライトの向きによるエッジライトの強さ。０で向きを関係なく強さ最大

/////////////////////////////
//影色パラメーター
float SColorBlend = 0.3;//影色のブレンド比率。最大１。0でデフォルトカラー

/////////////////////////////
//影パラメーター(セルフシャドウ部分)
float ShadowIntensityC = 0.7;//影の強さ。下げると暗くなる。照り返し環境光がない場合、０で真っ暗。
float ShadowIntensityE = 1.0;//影部分のエッジライト強さ。デフォ１。下げると弱くなる。

/////////////////////////////
//SSAOパラメーター
float SSAOPower = 0.7;//SSAOの強さ。
