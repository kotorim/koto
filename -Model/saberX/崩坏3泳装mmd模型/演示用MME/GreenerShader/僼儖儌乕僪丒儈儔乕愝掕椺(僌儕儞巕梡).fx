...
/////////////////////////////
//メインライトパラメーター
float3 TColorM = {1.00,1.00,0.80};//トップライトの色。//すべて1で白。すべて0で黒。
float3 MColorM = {0.80,0.80,1.00};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorM = {0.90,0.70,0.60};//ボトムライトの色。

float LightIntensityM = 0.95;//メインライトの全体の強さ。1で減少無し。
float TIntensityM = 1.0;//トップライトの強さ。1.0で強さの変化なし。
float MIntensityM = 0.4;//ミドルライトの強さ。1.0で強さの変化なし。
float UIntensityM = 0.2;//ボトムライトの強さ。1.0で強さの変化なし。

/////////////////////////////
//照り返し環境光パラメーター
float3 TColorA = {0.8,0.7,1.0};//トップライトの色。//すべて1で白。すべて0で黒。
float3 MColorA = {0.2,0.2,0.5};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorA = {0.1,0.3,0.7};//ボトムライトの色。

float LightIntensityA = 0.15;//照り返し環境光の全体の強さ。1で減少無し。
float TIntensityA = 2.0;//トップライトの強さ。1.0で強さの変化なし。
float MIntensityA = 1.0;//ミドルライトの強さ。1.0で強さの変化なし。
float UIntensityA = 2.0;//ボトムライトの強さ。1.0で強さの変化なし。
float AmbientBalance = 0.4;//環境光を加算か乗算か。１で加算。０で乗算

/////////////////////////////
//ハイライトパラメーター
int SpecularON = 0;//1で元のスペキュラーを使用。0で別計算のハイライトを使用。
float HilightPower = 0.4;//ハイライトの強さ。
float3 HiColor = {1.0,1.0,1.0};//ハイライトの色。すべて1で白。
float HilightGlow = 2;//ハイライトの広がり。数値を上げれば、ハイライトが狭くなる。基本2～3ぐらい。

/////////////////////////////
//エッジライトパラメーター(擬似バックライト)
float3 EColor = {1.0,1.0,1.0};
float LightIntensityE = 0.8;//エッジライト強さ自体の強さ
float AttenuationC = 0.7;//明るい所のエッジライト減少。０で減少無し。
float AttenuationD = 0.4;//ライトの向きによるエッジライトの強さ。０で向きを関係なく強さ最大

/////////////////////////////
//影色パラメーター
float SColorBlend = 1;//影色のブレンド比率。最大１。0でデフォルトカラー

/////////////////////////////
//影パラメーター(セルフシャドウ部分)
float ShadowIntensityC = 0.1;//影の強さ。下げると暗くなる。照り返し環境光がない場合、０で真っ暗。
float ShadowIntensityE = 0.8;//影部分のエッジライト強さ。デフォ１。下げると弱くなる。

/////////////////////////////
//SSAOパラメーター
float SSAOPower = 1.00;
...
#define USE_NORMAL_MAP // 使う? (使わない場合、左に//)
    #define NORMAL_MAP_PATH "_NM_Cloth(pmot氏).png"
  //#define INVERSE_X // 法線マップの X要素の方向を逆転
  //#define INVERSE_Y // 法線マップの Y要素の方向を逆転
    float2  NormalMapSize = float2(256, 256); // ノーマルマップのサイズ(視差遮蔽マップを使う場合、要指定)
...
#define USE_HEIGHT_MAP // 使う? (使わない場合、左に//)
  ...
  //#define INVERSE_HIGHT // 高さマップの高さを逆転 (通常:0低～1高, 逆転:1低～0高)
    ...
    float       HeightScalePOM = 0.03; // 深さ
    #define  POM_SMOOTHING_MIN   30   // 滑らかにするためのサンプル数：最小値(※増やすと超重くなります)
    #define  POM_SMOOTHING_MAX   60   //             〃              ：最大値(          〃          )
