...
float3 MColorM = {0.50,0.50,1.00};//ミドルライトの色。//色の明るさが強さに影響するので注意。
...
float3 UColorA = {1.0,0.4,0.2};//ボトムライトの色。

float LightIntensityA = 0.10;//照り返し環境光の全体の強さ。1で減少無し。
...
/////////////////////////////
//影パラメーター(セルフシャドウ部分)
float ShadowIntensityC = 0.5;//影の強さ。下げると暗くなる。照り返し環境光がない場合、０で真っ暗。
