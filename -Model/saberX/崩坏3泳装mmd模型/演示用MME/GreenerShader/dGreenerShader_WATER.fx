...
/////////////////////////////
//メインライトパラメーター
float3 TColorM = {0.80,0.80,1.00};//トップライトの色。//すべて1で白。すべて0で黒。
float3 MColorM = {0.25,0.25,1.00};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorM = {0.25,0.50,1.00};//ボトムライトの色。

float LightIntensityM = 0.90;//メインライトの全体の強さ。1で減少無し。
float TIntensityM = 1.0;//トップライトの強さ。1.0で強さの変化なし。
float MIntensityM = 0.7;//ミドルライトの強さ。1.0で強さの変化なし。
float UIntensityM = 0.3;//ボトムライトの強さ。1.0で強さの変化なし。

/////////////////////////////
//照り返し環境光パラメーター
float3 TColorA = {0.5,0.5,1.0};//トップライトの色。//すべて1で白。すべて0で黒。
float3 MColorA = {0.2,0.2,0.5};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorA = {0.5,0.3,0.7};//ボトムライトの色。

float LightIntensityA = 0.20;//照り返し環境光の全体の強さ。1で減少無し。
