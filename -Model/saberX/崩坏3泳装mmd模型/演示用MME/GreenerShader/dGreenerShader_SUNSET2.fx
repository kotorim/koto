...
/////////////////////////////
//メインライトパラメーター
float3 TColorM = {1.00,0.70,0.20};//トップライトの色。//すべて1で白。すべて0で黒。
float3 MColorM = {0.60,0.60,1.00};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorM = {0.50,0.60,0.50};//ボトムライトの色。

float LightIntensityM = 0.80;//メインライトの全体の強さ。1で減少無し。
...
/////////////////////////////
//照り返し環境光パラメーター
float3 TColorA = {0.3,0.5,1.0};//トップライトの色。//すべて1で白。すべて0で黒。
float3 MColorA = {1.0,0.6,0.3};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorA = {0.4,0.4,0.6};//ボトムライトの色。

float LightIntensityA = 0.3;//照り返し環境光の全体の強さ。1で減少無し。
float TIntensityA = 0.8;//トップライトの強さ。1.0で強さの変化なし。
