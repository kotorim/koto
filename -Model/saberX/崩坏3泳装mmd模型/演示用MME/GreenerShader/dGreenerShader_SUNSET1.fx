...
/////////////////////////////
//メインライトパラメーター
float3 TColorM = {1.00,0.70,0.30};//トップライトの色。//すべて1で白。すべて0で黒。
float3 MColorM = {0.60,0.60,1.00};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorM = {0.35,0.35,1.00};//ボトムライトの色。

float LightIntensityM = 0.90;//メインライトの全体の強さ。1で減少無し。
float TIntensityM = 1.0;//トップライトの強さ。1.0で強さの変化なし。
float MIntensityM = 0.5;//ミドルライトの強さ。1.0で強さの変化なし。
float UIntensityM = 0.3;//ボトムライトの強さ。1.0で強さの変化なし。

/////////////////////////////
//照り返し環境光パラメーター
float3 TColorA = {0.1,0.3,1.0};//トップライトの色。//すべて1で白。すべて0で黒。
float3 MColorA = {1.0,0.3,0.1};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorA = {0.4,0.4,0.8};//ボトムライトの色。

float LightIntensityA = 0.25;//照り返し環境光の全体の強さ。1で減少無し。
float TIntensityA = 0.6;//トップライトの強さ。1.0で強さの変化なし。
float MIntensityA = 2.0;//ミドルライトの強さ。1.0で強さの変化なし。
...
float3 HiColor = {1.0,0.9,0.7};//ハイライトの色。すべて1で白。
float HilightGlow = 2;//ハイライトの広がり。数値を上げれば、ハイライトが狭くなる。基本2～3ぐらい。

/////////////////////////////
//エッジライトパラメーター(擬似バックライト)
float3 EColor = {1.0,0.9,0.7};
float LightIntensityE = 1.2;//エッジライト強さ自体の強さ
float AttenuationC = 0.2;//明るい所のエッジライト減少。０で減少無し。
