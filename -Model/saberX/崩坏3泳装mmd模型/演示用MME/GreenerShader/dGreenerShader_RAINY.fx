...
float MIntensityM = 0.8;//ミドルライトの強さ。1.0で強さの変化なし。
float UIntensityM = 0.8;//ボトムライトの強さ。1.0で強さの変化なし。
...
float AmbientBalance = 0.8;//環境光を加算か乗算か。１で加算。０で乗算
...
float HilightGlow = 1;//ハイライトの広がり。数値を上げれば、ハイライトが狭くなる。基本2～3ぐらい。
...
/////////////////////////////
//影パラメーター(セルフシャドウ部分)
float ShadowIntensityC = 0.95;//影の強さ。下げると暗くなる。照り返し環境光がない場合、０で真っ暗。
