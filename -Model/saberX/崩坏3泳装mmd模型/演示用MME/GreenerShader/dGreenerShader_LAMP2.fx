...
/////////////////////////////
//メインライトパラメーター
float3 TColorM = {1.00,0.85,0.60};//トップライトの色。//すべて1で白。すべて0で黒。
float3 MColorM = {0.90,0.90,1.00};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorM = {0.80,0.80,1.00};//ボトムライトの色。

float LightIntensityM = 1.00;//メインライトの全体の強さ。1で減少無し。
float TIntensityM = 1.1;//トップライトの強さ。1.0で強さの変化なし。
float MIntensityM = 0.2;//ミドルライトの強さ。1.0で強さの変化なし。
float UIntensityM = 0.3;//ボトムライトの強さ。1.0で強さの変化なし。

/////////////////////////////
//照り返し環境光パラメーター
float3 TColorA = {1.0,0.5,0.2};//トップライトの色。//すべて1で白。すべて0で黒。
float3 MColorA = {0.3,0.2,0.6};//ミドルライトの色。//色の明るさが強さに影響するので注意。
float3 UColorA = {1.0,0.5,0.2};//ボトムライトの色。

float LightIntensityA = 0.4;//照り返し環境光の全体の強さ。1で減少無し。
float TIntensityA = 0.4;//トップライトの強さ。1.0で強さの変化なし。
float MIntensityA = 1.0;//ミドルライトの強さ。1.0で強さの変化なし。
float UIntensityA = 0.8;//ボトムライトの強さ。1.0で強さの変化なし。
float AmbientBalance = 0.5;//環境光を加算か乗算か。１で加算。０で乗算

/////////////////////////////
//ハイライトパラメーター
int SpecularON = 0;//1で元のスペキュラーを使用。0で別計算のハイライトを使用。
float HilightPower = 0.30;//ハイライトの強さ。
float3 HiColor = {1.0,0.85,0.6};//ハイライトの色。すべて1で白。
float HilightGlow = 1.5;//ハイライトの広がり。数値を上げれば、ハイライトが狭くなる。基本2～3ぐらい。

/////////////////////////////
//エッジライトパラメーター(擬似バックライト)
float3 EColor = {1.0,0.85,0.6};
float LightIntensityE = 0.5;//エッジライト強さ自体の強さ
float AttenuationC = 0.8;//明るい所のエッジライト減少。０で減少無し。
float AttenuationD = 0.7;//ライトの向きによるエッジライトの強さ。０で向きを関係なく強さ最大
...
/////////////////////////////
//影パラメーター(セルフシャドウ部分)
float ShadowIntensityC = 0.6;//影の強さ。下げると暗くなる。照り返し環境光がない場合、０で真っ暗。
float ShadowIntensityE = 0.8;//影部分のエッジライト強さ。デフォ１。下げると弱くなる。

/////////////////////////////
//SSAOパラメーター
float SSAOPower = 0.50;
